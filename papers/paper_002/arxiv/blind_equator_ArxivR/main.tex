...
Any $\theta \neq \pi/2$ breaks the cancellation. A grid search over $(\theta, \phi_2, \phi_3, \beta_{\rm Bob})$ maximizing $\text{FOM}(\theta, \beta, N) = \min(n_\sigma^{\rm LF}, n_\sigma^{\rm signal})$ with $\beta = 0.30$ yields a broad plateau peaking near $\theta \approx 31^\circ$--$35^\circ$ ($\text{FOM} = 8.6$--$8.8$). At $\beta = 0.30$, $n_\sigma^{\rm signal} \gg n_\sigma^{\rm LF}$ across all $\theta$, so the FOM is bounded by $n_\sigma^{\rm LF}$. Representative FOM values at $\mu = 0.95$, $\beta = 0.30$ (per-$\theta$ angle re-optimization, Supplemental~S2): $5.8$ ($\theta = 20^\circ$), $8.6$ ($\theta = 31^\circ$), $8.8$ ($\theta = 35^\circ$), $6.0$ ($\theta = 45^\circ$), $0$ ($\theta = 58^\circ$, Gen LF 1 becomes negative), and $0$ ($\theta = 90^\circ$, cancellation); $\text{FOM} > 5\sigma$ for $\theta \in [20^\circ, 45^\circ]$. At the minimum detectable $\beta \approx 0.07$, the FOM is signal-limited: optimal $\theta = 46^\circ$ ($\text{FOM} = 5.4$) and $>5\sigma$ range $\theta \in [35^\circ, 46^\circ]$ (Supplemental~S2).
...
The optimum at $\theta \approx 31^\circ$--$35^\circ$ reflects a trade-off between two monotonic trends.
As $\theta \to 0^\circ$, the overlap asymmetry is largest, but the Gen LF~1 violation
...
(equatorial cancellation, \S\ref{sec:theorem}). The intermediate
plateau emerges from the intersection of these competing
...
(FOM $> 5\sigma$ for $\theta \in [20^\circ, 45^\circ]$) means the exact optimum is not critical ---
