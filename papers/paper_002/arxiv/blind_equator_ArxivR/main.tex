...
% GitHub: https://github.com/AIhugART/ ; Facebook: https://www.facebook.com/xuanviet
% Have Optical Wigner's Friend Experiments Been Blind to a Geometric
% Degree of Freedom?
% Draft v99 — 2026-06-02
...
$\beta \sim 0.075$ at $5\sigma$ (single-setting) --- a search parameter whose methodological role parallels
...
$\delta\langle AB\rangle$ vanishes identically at $\theta = \pi/2$ and is generically non-zero
otherwise --- a genuine observable, not a coordinate artifact
...
$\delta\langle AB\rangle(\theta)$ is invariant under any basis redefinition (Lemma~1, \S\ref{sec:lemma}).
...
Superobserver measurement to $\theta = 31^\circ$, providing minimum detectable
$\beta \sim 0.075$ at $5\sigma$ (single-setting) while preserving $8.6\sigma$ Genuine LF
...
any $\theta \neq \pi/2$, with the $\theta$-dependent variation under $\theta$-sweep (\S\ref{sec:discussion}) providing the
distinguishing signature (vanishing identically at $\theta = \pi/2$ and determined
...
class. The distinctive experimental signature is that $\delta\langle AB\rangle$ vanishes
identically at $\theta = \pi/2$ and is generically non-zero for $\theta \neq \pi/2$ (exact
$\theta$-dependence determined numerically, \S\ref{sec:sensitivity}): this
structure is distinct from conventional systematics (which either cancel in
...
\emph{Proof.} Throughout, $\delta\langle AB\rangle \equiv \langle AB\rangle_{\rm model} - \langle AB\rangle_{\rm QM}$
denotes the deviation of the deformed model from standard QM (the same
quantity tabulated in \S\ref{sec:sensitivity}), not a bare correlator value.
Two distinct operations must be distinguished. (i)~\emph{Passive
relabeling}: redefining outcome labels via $|b'\rangle = U|b\rangle$ is a change
of description, not a change of the measurement --- it acts identically on the
deformed model and on standard QM. It therefore cannot generate any
model--QM deviation: $\delta\langle AB\rangle = 0$ identically for any
...
$\delta\langle AB\rangle \neq 0$ for $\theta \neq \pi/2$. The two are operationally distinct. $\square$
...
via $U = \sigma_x$ --- flips the sign of each individual correlator
($\langle AB\rangle \to -\langle AB\rangle$) but acts identically on the model and on QM, leaving
their difference at zero ($\delta\langle AB\rangle = 0$); Eq.~(\ref{eq:model_class}), by contrast, still
predicts a nonzero deviation $\delta\langle AB\rangle \approx 0.008$. The two
...
of standard KS contextuality --- it concerns measurement \emph{record}
...
\begin{tabular}{p{0.18\columnwidth}p{0.24\columnwidth}p{0.26\columnwidth}p{0.24\columnwidth}}
\toprule
...
Observable & Outcome distributions across incompatible settings & $\delta\langle AB\rangle(\theta)$ vanishing iff $\theta = \pi/2$, at fixed setting & Weak value $A_w$ \\
...
preferentially with one Friend outcome. Lower panel: predicted $\delta\langle AB\rangle(\theta)$ curve
(numerical), showing the exact null at $\theta = 90^\circ$ (equator) and the onset of non-zero
signal as $\theta$ departs from the equatorial plane. The single-waveplate modification
...
object is this conditional probability, not a single-photon overlap $|\langle b|d\rangle|^2$;
the value $1/2$ plays the same role here as the equatorial overlap $|\langle b|d\rangle|^2 = 1/2$
derived in \S\ref{sec:proof}, so the BSM configuration is effectively equatorial. Full
...
Any $\theta \neq \pi/2$ breaks the cancellation. A grid search over $(\theta, \phi_2, \phi_3, \beta_{\rm Bob})$ maximizing $\text{FOM}(\theta, \beta, N) = \min(n_\sigma^{\rm LF}, n_\sigma^{\rm signal})$ with $\beta = 0.30$ yields a broad plateau peaking near $\theta \approx 31^\circ$--$35^\circ$ ($\text{FOM} = 8.6$--$8.8$). At $\beta = 0.30$, $n_\sigma^{\rm signal} \gg n_\sigma^{\rm LF}$ across all $\theta$, so the FOM is bounded by $n_\sigma^{\rm LF}$. Representative FOM values at $\mu = 0.95$, $\beta = 0.30$ (per-$\theta$ angle re-optimization, Supplemental~S2): $5.8$ ($\theta = 20^\circ$), $8.6$ ($\theta = 31^\circ$), $8.8$ ($\theta = 35^\circ$), $6.0$ ($\theta = 45^\circ$), $0$ ($\theta = 58^\circ$, Gen LF 1 becomes negative), and $0$ ($\theta = 90^\circ$, cancellation); $\text{FOM} > 5\sigma$ for $\theta \in [20^\circ, 45^\circ]$. At $\beta \approx 0.07$ (single-setting, just below the single-setting $5\sigma$ threshold $\beta \approx 0.075$), the FOM is signal-limited: optimal $\theta = 46^\circ$ ($\text{FOM} = 5.4$) and single-setting $>5\sigma$ range $\theta \in [35^\circ, 46^\circ]$ (Supplemental~S2). The reference angle $\theta = 31^\circ$ gives single-setting significance $4.7\sigma$ at $\beta = 0.07$; reaching $5\sigma$ there requires $\beta \gtrsim 0.075$ (single-setting) or combining the four mixed settings ($9.4\sigma$ at $\beta = 0.07$, \S\ref{sec:sensitivity}). The $\beta = 0.30$ plateau $\theta \in [20^\circ, 45^\circ]$ does include $\theta = 31^\circ$.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig2_fom_vs_theta.png}
\caption{Figure of merit vs polar angle $\theta$, showing plateau near $\theta \approx 31^\circ$--$35^\circ$ ($\beta = 0.30$) and $5\sigma$ detection boundary spanning $\theta \in [20^\circ, 45^\circ]$.}
\label{fig:fom_theta}
\end{figure}
...
The optimum at $\theta \approx 31^\circ$--$35^\circ$ reflects how the two significances vary with $\theta$.
In this tilted-Superobserver geometry the Gen LF~1 violation is itself maximized at an
intermediate tilt: it weakens as $\theta \to 0^\circ$ (settings~2,3 collapse onto the $z$-basis
of setting~1, removing the angular spread the Facet~1 inequality requires) and also weakens
toward the equator, the achievable violation falling to zero near $\theta \approx 55^\circ$ (Gen LF~1
turns negative by $\theta \approx 58^\circ$; Supplemental~S2 grid search). The overlap signal
independently vanishes at $\theta = 90^\circ$ (equatorial cancellation, \S\ref{sec:theorem}) and is
non-zero otherwise. At $\beta = 0.30$ the FOM is LF-limited, so its $\theta \approx 31^\circ$--$35^\circ$
plateau tracks the peak of the Gen LF~1 violation; at $\beta \approx 0.07$ the FOM is signal-limited
and the optimum shifts toward $\theta \approx 46^\circ$. The exact location is set by the Gen LF~1
inequality coefficients via grid search (Supplemental~S2). The broad plateau
(FOM $> 5\sigma$ for $\theta \in [20^\circ, 45^\circ]$ at $\beta = 0.30$) means the exact optimum is not critical ---
any angle in this range produces a viable experiment. We adopt $\theta = 31^\circ$ as the reference angle throughout
...
shift in Gen LF~1 without the $\theta$-dependent pattern in $\delta\langle AB\rangle$ would indicate
...
$\mu$ required (onset)   & not specified & $\ge 0.86$ \\
$N$                      & 91{,}000    & 91{,}000 \\
\bottomrule
\end{tabular}
\end{table}
...
\item Measure $\mu$ via CHSH $S$-parameter ($\mu \ge 0.86$ required for LF violation onset).
\end{enumerate}
...
$\rho_\mu = \mu|\Psi^-\rangle\langle\Psi^-| + (1-\mu)(|HV\rangle\langle HV| + |VH\rangle\langle VH|)/2$ for the
singlet state with visibility $\mu = 0.95$. SPDC produces photon pairs only in the
$\{|HV\rangle, |VH\rangle\}$ subspace; the noise term is the maximally mixed state
within that subspace, not the full $I/4$.
...
\textbf{Experimental discriminator.} Standard QM predicts $\delta\langle AB\rangle = 0$ for all $\theta$. Eq.~(\ref{eq:model_class})--(\ref{eq:fperp}) predicts $\delta\langle AB\rangle = 0$ at $\theta = \pi/2$ (equatorial cancellation) and $\delta\langle AB\rangle \neq 0$ for $\theta \neq \pi/2$, with exact $\theta$-dependence determined numerically (the unrenormalized leading-order structure goes as $\cos\theta$, but renormalization modifies the functional form; see Supplemental~S2). This signature is testable by $\theta$-sweep (\S\ref{sec:discussion}) and is not a reparameterization of QM: it is distinct from conventional systematics (which either cancel in $\delta\langle AB\rangle$ or produce non-geometric $\theta$-dependence), making a $\theta$-dependent $\delta\langle AB\rangle$ signal difficult to reproduce without overlap-dependent physics (Lemma~1, \S\ref{sec:lemma}).
...
The $\sim\!10^{-2}$ scale is
physically motivated: postselection-conditioned weak values~\cite{Aharonov1988} manifest
at the same order, and any overlap-dependent structure would naturally appear at the precision where
...
(11~terms, coefficients up to $\pm 2$): $\sigma(S_{\rm LF1}) = \sqrt{\sum_i c_i^2(1 - \langle v_i\rangle^2)/N} \approx 0.0103$
at $N = 91{,}000$ (exact term-by-term propagation; the upper bound $\sqrt{20}/\sqrt{N} \approx 0.015$
assumes all correlators near zero).

Minimum sample for $5\sigma$ LF detection: $N_{\rm min} \approx 30{,}800$. $N = 91{,}000$ provides a
factor of 3 margin.

Monte Carlo (10{,}000~runs): Gen LF~1 $\ge 5\sigma$ in $99.97\%$; $\beta = 0.07$ detected in
$\sim\!38\%$ at $5\sigma$ (single setting, $n_\sigma = 4.7$; see \S\ref{sec:sensitivity}),
${>}99\%$ (four combined settings, $n_\sigma = 9.4$); $\beta = 0.05$ in $\sim\!90\%$ at $5\sigma$
(combined, $n_\sigma = 6.7$). A conservative Bayesian analysis
inflating Poisson uncertainties by $20\%$ yields $\beta_{\rm min} \approx 0.046$ (combined); the FOM
plateau (\S\ref{sec:breaking}: ${>}5\sigma$ for $\theta \in [20^\circ, 45^\circ]$) ensures viability under substantial
...
Required loophole-closed $5\sigma$ detection thresholds (visibility $\mu \ge 0.92$,
efficiency $\eta \ge 0.91$) are within reach
...
\begin{tabular}{p{0.29\columnwidth}p{0.44\columnwidth}p{0.15\columnwidth}}
\toprule
...
$\delta\langle AB\rangle(\phi) = A + B\cos(2\phi) + C\sin(2\phi)$) distinguishes geometric $\theta$-dependent
...
The overlap-only class is definitively falsified if: (i)~a $\theta$-sweep over
the pre-registered angle set $\{20^\circ, 31^\circ, 35^\circ, 45^\circ, 58^\circ\}$ shows $\delta\langle AB\rangle = 0$ at all
these non-equatorial angles to within $\pm 0.003$ ($90^\circ$ serves as the equatorial control;
statistical floor at $N = 200{,}000$ per setting); or
(ii)~$\delta\langle AB\rangle(\theta)$ is non-zero at $\theta = \pi/2$ (violating the equatorial cancellation theorem) after
accounting for systematics. Either outcome is informative: falsification closes the
overlap-only window; a $\theta$-dependent signal opens it.
...
\small
\begin{tabular}{p{0.26\columnwidth}p{0.28\columnwidth}p{0.38\columnwidth}}
\toprule
...
$\delta\langle AB\rangle$ at $\theta = 31^\circ$ & $0$ & $\approx 0.115\beta$ (numerical) \\
$\delta\langle AB\rangle$ at $\theta = \pi/2$ & $0$ & $0$ (equatorial cancellation, exact) \\
$\delta\langle AB\rangle(\theta)$ functional form & $\delta = 0$ $\forall\theta$ & $\delta = 0$ iff $\theta = \pi/2$; non-zero otherwise (exact form numerical) \\
\bottomrule
\end{tabular}
\end{table}

A null result at Level~0 ($\beta < \beta_{\rm min}$ across the full $\theta$-sweep) falsifies the overlap-only class but leaves Levels~1--3 unconstrained: density-matrix-dependent ($\rho_F$), multi-partite (concurrence), and non-geometric (timing, path) deformations are not bounded by Proposition~1 and each requires independent experimental designs.

\subsection{Future Directions}
\label{sec:future}

\textbf{$\theta$-sweep.} A systematic scan over the pre-registered angle set
$\{20^\circ, 31^\circ, 35^\circ, 45^\circ, 58^\circ, 90^\circ\}$ ($90^\circ$ = equatorial control) would
directly map the $\theta$-dependence of $\delta\langle AB\rangle$ predicted by Eq.~(\ref{eq:model_class}), testing
for the equatorial zero ($\delta = 0$ at $\theta = 90^\circ$) and non-zero signal at
$\theta \neq 90^\circ$. To prevent
analysis bias, the sweep should be performed blind (randomized $\theta$ sequence,
analysis finalized before unmasking). A null result across all $\theta$
excludes the overlap-only class to $\beta \approx 0.02$ ($N = 200{,}000$ per setting).

\textbf{Multi-observer extension.} Extension to $N > 2$ observers is a natural
...
Bong et al.\ (2020) apparatus ($\theta = 31^\circ$), providing sensitivity $\beta \sim 0.075$
