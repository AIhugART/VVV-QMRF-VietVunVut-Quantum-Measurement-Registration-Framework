...
\subsection{Reproducible Code Index}
All numerical predictions are reproducible via the scripts in the \texttt{papers/paper\_002/supplemental/} directory.

\subsubsection*{Scripts}
\begin{table}[h]
\centering
\caption{Replication script inventory.}
\begin{tabular}{lll}
\toprule
Script & Purpose & Paper Section \\
\midrule
\texttt{K9S12\_proposal.py} & Full protocol: angle optimization, correlator table, $K9\_E$ predictions & 4--5 \\
\texttt{statistical\_significance.py} & FOM scan, Monte Carlo, $\sigma$ computations & 6 \\
\texttt{RCA\_full\_verification\_v93.py} & Complete RCA verification of all manuscript v93 claims & All \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection*{Reproducing key numbers}
To reproduce the primary predictions:
\begin{verbatim}
...
Python 3.9+, \texttt{numpy}, \texttt{scipy}. No external data files are needed (all parameters are inline).

\subsubsection*{$K9\_E$ deformation model}
The manuscript (Eq.~(2)--(3)) uses the overlap-dependent form:
\begin{equation}
P_{\rm K9E}(a,b|x,y) = P_{\rm QM}(a,b|x,y) \cdot \frac{1 - \beta f_\perp(b,d)}{Z}
\end{equation}
where $f_\perp(b,d) = 1 - |\langle b|d\rangle|^2$ and $Z$ is the normalization factor. This acts at the probability level, modifying outcome weights by the geometric overlap between Superobserver outcome $b$ and Friend outcome $d$.

\subsubsection*{Model parameters}
$N = 91{,}000$ (Bong 2020), $\lambda = 810$~nm SPDC, $\mu = 0.95$ (nominal), $\mu_{\rm threshold} = 0.86$.
