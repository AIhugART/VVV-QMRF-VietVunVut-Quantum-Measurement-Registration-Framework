...
**Scoring summary (1 finding):** 1 implemented (avg 4.93/5 ≥ 4.0).

| # | Finding (symptom) | 5-Why root cause | R1 | R2 | R3 | Avg | Action |
|---|-------------------|------------------|----|----|----|-----|--------|
| 1 | §5 density matrix writes the signal/source state as ρ_μ = μ**\|Φ⁻⟩⟨Φ⁻\|** + (1−μ)/2(\|HV⟩⟨HV\|+\|VH⟩⟨VH\|), but \|Φ⁻⟩ = (\|HH⟩−\|VV⟩)/√2 lives in the {HH,VV} subspace — contradicting (a) the very next sentence "SPDC produces photon pairs only in the {\|HV⟩,\|VH⟩} subspace", (b) the noise term placed in {HV,VH}, (c) the reported ⟨A₁B₁⟩ = −1.0000 (z-basis perfect **anti**-correlation), and (d) the −cosθ mixed correlators / their μ-independence. | The I/4→{HV,VH} noise-model correction (review round, `review/walkthrough.md` + `review/RCA_manuscript_analysis.md`, ≈v92) fixed the *noise* term but left/typed the *signal* ket as \|Φ⁻⟩, conflating "the singlet" with \|Φ⁻⟩. The singlet is \|Ψ⁻⟩ = (\|HV⟩−\|VH⟩)/√2; \|Φ⁻⟩ is a {HH,VV} (triplet-class) Bell state. The numbers were generated by code (`K9S12_proposal.py`) using the singlet, so the tables were always right — only the prose ket was wrong, so no numeric check ever flagged it. Prior RCA rounds (v94–v96) audited thresholds (β, σ, FOM), not ket notation. | 4.9 | 4.9 | 5.0 | **4.93** | **Implemented** — \|Φ⁻⟩ → \|Ψ⁻⟩ in every ρ_μ definition (signal state); BSM four-Bell-state enumerations [a] (which legitimately list \|Φ⁻⟩) left intact |

### Numerical verification (state diagnostics — confirm |Ψ⁻⟩, refute |Φ⁻⟩)

- **z-basis correlator:** \|Ψ⁻⟩ gives ⟨A₁B₁⟩ = −1 (both \|HV⟩ and \|VH⟩ → AB = −1); the {HV,VH} noise term also gives −1 ⇒ total = −1.0000 ∀μ ✓ (matches reported value, σ=0.0000). \|Φ⁻⟩ would give ⟨A₁B₁⟩ = +1 (signal) → mixture 2μ−1 = +0.90 ✗.
- **Mixed correlator:** \|Ψ⁻⟩ is rotationally invariant ⇒ ⟨A₁B₂⟩ = −cosθ = −cos31° = −0.8572 ✓; noise also yields −cosθ ⇒ μ-independence of the mixed correlator (Supplemental S2.2) holds ✓. \|Φ⁻⟩ is **not** rotationally invariant ⇒ generic −cosθ would fail ✗.
- Conclusion: all tabulated correlators are self-consistent **only** under \|Ψ⁻⟩. No table value changes; the fix is purely the ket symbol.

### Files changed (source + derived + SOT kept in sync)

| File | Edit |
|------|------|
| `manuscript.md` (source) | §5 ρ_μ ket \|Φ⁻⟩→\|Ψ⁻⟩; Status → v97 (round 7) |
| `arxiv/blind_equator_ArxivR/main.tex` | §5 ρ_μ ket \|Φ⁻⟩→\|Ψ⁻⟩; header → v97 |
| `arxiv/blind_equator_ArxivR/supplemental.tex` | S2 ρ_μ ket \|Φ⁻⟩→\|Ψ⁻⟩ |
| `arxiv/blind_equator_ArxivR/README.md` | main.tex version → v97 |
| `SOT/paper_002_SOT.md` | §6.1 + footer ρ_μ ket \|Phi-⟩→\|Psi-⟩ (2 sites) |
| `SOT/export/sot_export.tex` | ρ_μ \ketbra{\Phi^-}→{\Psi^-} (2 sites) |
| `arxiv/blind_equator_ArxivR.md` (stale v95 duplicate) | §5 ρ_μ ket \|Φ⁻⟩→\|Ψ⁻⟩ (physics fix only) |

### Notes / regression

- **Not changed (intentional):** BSM four-Bell-state lists `{|Φ⁺⟩,|Φ⁻⟩,|Ψ⁺⟩,|Ψ⁻⟩}` in footnote [a] (manuscript §3.6, main.tex, supplemental S1.2) — \|Φ⁻⟩ is correct there. No theorem, claim class, β/σ/FOM value, or correlator table altered. Proposition 1 / Lemma 1 untouched.
- **Out of scope (flagged):** `review/walkthrough.md` (L56) and `review/RCA_manuscript_analysis.md` (L32) record the original \|Φ⁻⟩ choice and the (incorrect) claim that it yields ⟨A₁B₁⟩=−1.0000; these are historical RCA records, left as-is. `paper_002/README.md` is independently stale (still v94/v95) and was not re-synced by this round.
- PDFs not recompiled this session (no LaTeX toolchain invoked); source `.tex` edits are single-token and compile-safe.
