...
**English:**
Currently, QM handles a detector failing to click (even when a particle passes through) via a statistical parameter called *Detection Efficiency* ($\eta$). Under this new category, QM structures it as a registration mechanism:
...
* **EN:** The core characteristic of this category is the absolute dissociation between causal presence and registration determination. The quantum particle has truly impacted the macroscopic world, but the macroscopic registration chain has not encoded it.
* **VN:** Đặc tính cốt lõi của phạm trù này là sự phân ly tuyệt đối giữa sự tồn tại nhân quả (causal presence) và sự xác định ghi nhận (registration determination). Hạt lượng tử đã thực sự tác động lên thế giới vĩ mô, nhưng chuỗi ghi nhận vĩ mô chưa mã hóa nó.
...
* **EN:** 
  ...
  - **BIAN-13 (*Anadhyavasāya*):** *Yes* physical interaction, but *No* information (Null Registering-System Event).
  $\rightarrow$ Together, these two categories form a perfect Registration-Physical matrix.
* **VN:** 
  - **BIAN-15 (*Kevalavyatirekin*):** *Không* có tương tác vật lý, nhưng *Có* sinh ra thông tin tuyệt đối (Phép đo phi tương tác).
  - **BIAN-13 (*Anadhyavasāya*):** *Có* tương tác vật lý, nhưng *Không* sinh ra thông tin (Sự kiện Hệ ghi nhận Rỗng).
  $\rightarrow$ Hai phạm trù này ghép lại tạo thành một ma trận Ghi nhận - Vật lý hoàn hảo.

### 4.3. Absolute Distinction from Doubt and Error / Khác biệt tuyệt đối với "Nghi ngờ" và "Sai lầm"
* **EN:** In this state, the registering system is neither suspended between two outcomes (not *Saṃśaya* / QM Superposition), nor encoding a false detector response (not *Bhrānti* / QM Artifact). The registering system remains in complete informational silence.
* **VN:** Trong trạng thái này, hệ ghi nhận không treo giữa 2 kết quả (không phải *Saṃśaya* / QM Superposition), cũng không mã hóa sai một detector response (không phải *Bhrānti* / QM Artifact). Hệ ghi nhận ở trạng thái tĩnh lặng hoàn toàn về mặt thông tin.
