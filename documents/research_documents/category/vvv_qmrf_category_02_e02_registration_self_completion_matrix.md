...
flowchart TD
    subgraph BE["Buddhist Epistemology source layer"]
        BE1["*Pramāṇa-Phala Identity* as source analogue; no dedicated current BE node"]
    end

    subgraph QM["Standard Quantum Measurement substrate"]
        QM1["measurement act, system-meter coupling, projection support, Born-rule state update"]
    end

    subgraph VVV["VVV-QMRF Registration Layer"]
        CAT["Category 02 / E2<br/>Registration Self-Completion Matrix / Act-Result Registration Identity"]
        SYM["Irreducible Event Tensor `𝒯_act-res`"]
        BOUND["RCA boundary<br/>not a replacement for the Born Rule and not a claim that physical dynamics and semantic registration are the same level"]
    end
