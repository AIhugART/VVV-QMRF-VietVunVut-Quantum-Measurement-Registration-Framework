...
| 17 | **Registration Capacity Postulate (K10_R) — Layer 3** | **Class C (qualified) — structurally testable, empirically UNCONFIRMED (2026-05-31)** — C₁ (Admission Capacity ← K1), C₂ (Binary Validity ← K4), C₃ (Temporal Ordering ← K2), C₄ (Self-Certification ← K3). Grounds ValidReg(X,R) Condition 2. Handles N_QM_VVV_00038. Prerequisite for K9_E. Testability: C₁-threshold protocol via N_QM_00021; disconfirmation via N_QM_VVV_00038 (ρ=3). "Qualified": C₄ lacks K∩ρ EX anchor. Adopted Class D (RCA 4.60/5) → promoted Class C (RCA 4.67/5). Full text: `meta_architecture/K10_R_Registration_Capacity_Postulate.md`. | **High** |
